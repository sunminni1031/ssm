    ...
    def m_step(self, expectations, datas, inputs, masks, tags, **kwargs):
        ...
        for k in range(K):
            if self.bias:
                ...
                b = (Ey[k] - a * Ex[k]) / En[k]
                sigma = ((a ** 2) * ExTx[k] + En[k] * b @ b + EyTy[k] + 2 * a * Ex[k] @ b - 2 * a * ExTy[k] - 2 * Ey[
                    k] @ b) / (2 * En[k])
            else:
                a = ExTy[k] / (ExTx[k] + j)
                b = np.zeros((D,))
...
class ScalarAutoRegressiveSegFit(ScalarAutoRegressiveObservationsNoInput):
    ...
    @property
    def params(self):
        return self.a_arr, self.b_arr, self.sigma_arr
    ...
    def m_step(self, expectations, datas, inputs, masks, tags, **kwargs):
        ...
        for (Ez, _, _), data, input in zip(expectations, datas, inputs):
            ...
            for iseg in range(len(chng_idxs) - 1):
                ...
                Ex = np.einsum('t,ti->i', w[1:], x[:-1])
                Ey = np.einsum('t,ti->i', w[1:], x[1:])
                E1 = np.sum(w[1:])
                a = (ExTy * E1 - Ex @ Ey) / (ExTx * E1 - Ex @ Ex)
                b = (Ey - a * Ex) / E1
                sigma = ((a ** 2) * ExTx + E1 * b @ b + EyTy + 2 * a * Ex @ b - 2 * a * ExTy - 2 * Ey @ b) / (2 * E1)
                a_arr.append(a)
                b_arr.append(b)
                sigma_arr.append(sigma)
        self.a_arr, self.b_arr, self.sigma_arr = a_arr, b_arr, sigma_arr
        return

    def sample_x(self, iseg, xhist, input=None, tag=None, with_noise=True, rs=None):
        tmp_rs = npr if rs is None else rs
        a, b, sigma = self.a_arr[iseg], self.b_arr[iseg], self.sigma_arr[iseg]
        if xhist.shape[0] < 1:
...
class ScalarAutoRegressiveFlexibleBias(ScalarAutoRegressiveSegFit):
    def __init__(self, K, D, M=1, lags=1, bias=True, seed=0, **kwargs):
        self.K, self.D, self.M = K, D, M
        self.rs = npr.RandomState(seed)
        assert (K == 2) & (M == 1) & (lags == 1) & bias
    ...
    @property
    def params(self):
        return self.ak, self.b_arr, self.sigmak

    @params.setter
    def params(self, value):
        self.ak, self.b_arr, self.sigmak = value
